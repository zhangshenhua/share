...
static short preferred_width(HWidget *p_Me){
	...
	i_tp_one = p_Me->uc_padding_left +p_Me->uc_padding_right + p_this->text_width;
...
static short preferred_height(HWidget *p_Me){
	...
	i_tp_one = vm_graphic_get_character_height() + p_Me->uc_padding_bottom + p_Me->uc_padding_top;
	if (p_this->bg_image){
...
static void draw(HWidget *p_widget, int i_layer, short s_x, short s_y){
	...
	if (p_Me->text){
		if ((unsigned char)1 == p_Me->uc_ismid){
			...
			s_p_tp.s_x = s_x + p_widget->uc_padding_left;
			s_p_tp.s_y = s_y + p_widget->uc_padding_top;
		}
...
HPushButton *hpushbutton_new(const char *pc_str){
	...
	p_parent->uc_padding_top = 6;
	p_parent->uc_padding_right = 6;
	p_parent->uc_padding_left = 6;
	p_parent->uc_padding_bottom = 6;
	p_parent->p_widget_ops->set_enable_bgcolor(p_parent, 1);
	p_parent->i_bgcolor = 0x0000;
